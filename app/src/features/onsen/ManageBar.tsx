...
const ManageBar = ({ farm }) => {
  ...
  const balanceFiatValue = CurrencyAmount.fromRawAmount(
    ...
    farm.pair.type === PairType.KASHI
      ...
      : JSBI.BigInt(
          (
            (Number(balance?.toExact() ?? "0") * farm.pair.reserveUSD) /
          ...
          <div className="flex items-center">
            ...
            >
              <span
                className={`${
                  toggle
                    ? "translate-x-[1px] text-blue"
                    : "translate-x-[23px] text-pink"
          ...
          {["25", "50", "75", "100"].map((multipler, i) => (
            <Button
              ...
              onClick={() => {
                toggle
                  ? setDepositValue(
                      balance.multiply(multipler).divide(100).toExact()
                    )
                  : setWithdrawValue(
                      stakedAmount.multiply(multipler).divide(100).toExact()
                    );
              }}
