...
import { Decimal, Zoo } from '../src'
import { Blockchain, generatedWallets } from '@zoolabs/core/dist/utils'
import { JsonRpcProvider } from '@ethersproject/providers'
import { mintCurrency, setupZoo, ZooConfiguredAddresses } from './helpers'
import { BaseErc20Factory } from '@zoolabs/core/dist/typechain'
  ...
  describe('EIP-712 Utilities', () => {
    describe('#signPermitMessage', () => {
      ...
      it('signs a permit message that is able to be processed on chain', async () => {
        const provider = new JsonRpcProvider()
        const [mainWallet, otherWallet] = generatedWallets(provider)
        const onChainConfig = await setupZoo(mainWallet, [otherWallet])
        const mainZoo = new Zoo(
          mainWallet,
          50,
          onChainConfig.media,
        ...
        await mainZoo.mint(mediaData, bidShares)
    ...
    describe('#signMintWithSig', () => {
      ...
      it('signs a mintWithSig message that is able to be processed on chain', async () => {
        ...
        await otherZoo.mintWithSig(mainWallet.address, mediaData, bidShares, eipSig)
