...
const SetBid = ({ tokenId, children }) => {
  ...
  const [approvalState, approve] = useApproveCallback(CurrencyAmount.fromRawAmount(currencyToken, ask?.amount || 0), market.address)
  ...
  return (
    <div className='m-auto sm:p-4 md:p-0 w-96'>
      <div className='mb-10 text-right'>
        <div className={`${formattedBalance === '0' ? 'text-red' : 'text-gray-500'}`}>{account && shortenAddress(account)}</div>
      ...
      {(approvalState === ApprovalState.APPROVED || currencyToken?.isNative) && formattedBalance !== '0' && (
        <SetBidButton ask={ask} tokenType={type} tokenId={tokenId} amount={0} currencyToken={currencyToken} />
      )}
      <div className='pt-3'>
        {pendingTx ? (
