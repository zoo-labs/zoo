...
export function useAsset(tokenId: number | string) {
  ...
  const [currencyToken, setCurrencyToken] = useState(
    new Token(chainId, ethers.constants.AddressZero, 18) as Currency
  );
...
export const usePrice = () => {
  ...
  const COINGECKO_API_V3 = `https://api.coingecko.com/api/v3/simple/price?ids=${coinIds.join(
    ","
  )}&vs_currencies=usd`;
  ...
  const getUsdAmount = (
    tokenAddress: string,
    tokenAmount: BigintIsh
  ): string => {
    const currencyToken =
      getCurrencyTokenLowerCase(tokenAddress, chainId) ||
      new Token(chainId, ethers.constants.AddressZero, 2);
    const usdPrice = getUsdPrice(currencyToken?.symbol);
    const amount = formatCurrencyFromRawAmount(currencyToken, tokenAmount);
    return usdPrice
      ? numberWithCommas((parseFloat(amount) * usdPrice).toFixed(0))
