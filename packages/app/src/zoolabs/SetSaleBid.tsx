...
import { formatCurrencyAmount, formatCurrencyFromRawAmount, numberWithCommas, shortenAddress, shortenString } from '../functions'
...
const SetSaleBid = ({ dropId, name, children }) => {
  ...
  const [approvalState, approve] = useApproveCallback(CurrencyAmount.fromRawAmount(selectedCurrencyToken || currencyToken, ask?.amount || 0), market.address)
  ...
  return (
    <div className='m-auto sm:p-4 md:p-0 w-96'>
      <div className='mb-10 text-right'>
        <div className={`${formattedBalance === '0' && !offlineBidder ? 'text-red' : 'text-gray-500'}`}>{account && shortenAddress(account)}</div>
      ...
      {(formattedBalance === '0' && !offlineBidder) || isInsufficient ? (
        ...
        <>
          {' '}
          {!currencyToken?.isNative && [ApprovalState.NOT_APPROVED, ApprovalState.UNKNOWN].includes(approvalState) && formattedBalance !== '0' && (
            <div>
              <button
                type='button'
          ...
          {!isInsufficient && (approvalState === ApprovalState.APPROVED || currencyToken?.isNative || offlineBidder) && (formattedBalance !== '0' || offlineBidder) && (
            <SetSaleBidButton ask={ask} dropId={dropId} name={name} amount={0} currencyToken={currencyToken} isAllowed={isValid} />
          )}
        </>
      )}
      ...
      <div className='pt-3'>
        {pendingTx && (
          <p className='cursor-pointer '>
            <a href={getExplorerLink(chainId, pendingTx, 'transaction')} target='_blockchain_browser' className='flex justify-center'>
