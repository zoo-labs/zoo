...
import TransactionConfirmationModal from '../TransactionConfirmation' // , { ConfirmationModalContent, TransactionErrorContent }
 ...
 */
function tradeMeaningfullyDiffers(...args: [V2Trade<Currency, Currency, TradeType>, V2Trade<Currency, Currency, TradeType>]): boolean {
...
export default function ConfirmSwapModal({
  ...
  const modalBottom = useCallback(() => {
    return trade ? <SwapModalFooter onConfirm={onConfirm} trade={trade} disabledConfirm={showAcceptChanges} swapErrorMessage={swapErrorMessage} /> : null
  ...
  const pendingText2 = minerBribe
    ? trade?.outputAmount.currency.isNative
      ? `Minus ${CurrencyAmount.fromRawAmount(Ether.onChain(ChainId.MAINNET), minerBribe).toSignificant(6)} ETH Miner Tip`
