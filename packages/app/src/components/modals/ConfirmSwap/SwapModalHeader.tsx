...
import { Currency, Percent, TradeType, Trade as V2Trade } from '@zoolabs/sdk'
...
export default function SwapModalHeader({
  ...
  return (
    <div className='grid gap-4'>
      <div className='grid gap-2'>
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center gap-3'>
            ...
            <div className={`overflow-ellipsis w-[220px] overflow-hidden font-bold text-2xl ${priceImpactSeverity > 2 ? 'text-red' : 'text-high-emphesis'}`}>
      ...
      <TradePrice price={trade.executionPrice} showInverted={showInverted} setShowInverted={setShowInverted} className='px-0' />
      ...
      {recipient !== null ? (
        <div className='flex-start'>
          <>
            {i18n._(t`Output will be sent to`)} <b title={recipient}>{isAddress(recipient) ? shortenAddress(recipient) : recipient}</b>
          </>
        </div>
      ) : null}
    </div>
