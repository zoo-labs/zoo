...
function ImportList({ listURL, list, setModalView, onDismiss }: ImportProps) {
  ...
  return (
    <div>
      <ModalHeader onClose={onDismiss} title={i18n._(t`Import List`)} onBack={() => setModalView(CurrencyModalView.manage)} />
      <div className='px-1 space-y-4'>
        <div className='flex flex-row items-center px-4'>
          ...
          <div style={{ marginLeft: '20px' }}>
            ...
            <a className='overflow-hidden' href={`https://tokenlists.org/token-list?url=${listURL}`}>
        ...
        <div className='space-y-2'>
          ...
          <div className='flex flex-col mb-3 space-y-4 text-center whitespace-pre-line'>
            <div className='font-semibold text-red'>
              By adding this list you are implicitly trusting that the data is correct. Anyone can create a list, including creating fake versions of existing lists and lists that
              claim to represent projects that do not have one.
            </div>
            <div className='font-semibold text-red'>If you purchase a token from this list, you may not be able to sell it back.</div>
          </div>
          <div className='flex flex-row justify-center cursor-pointer' onClick={() => setConfirmed(!confirmed)}>
            <Checkbox name='confirmed' checked={confirmed} onChange={() => setConfirmed(!confirmed)} />
