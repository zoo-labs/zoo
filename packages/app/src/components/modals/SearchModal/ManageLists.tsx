...
const ListRow = memo(({ listUrl }: { listUrl: string }) => {
  ...
  return (
    <div
      ...
      className={`
      ${isActive ? 'text-high-emphesis' : 'text-primary bg-dark-700 '}  rounded flex flex-row p-4
      `}
      key={listUrl}>
      {list.logoURI ? <ListLogo size='40px' logoURI={list.logoURI} alt={`${list.name} list logo`} /> : <div style={{ width: '24px', height: '24px' }} />}
      <div className='justify-center flex-auto ml-4'>
        <div>
          <div
            className={`
          ${isActive && 'text-white'} overflow-hidden overflow-ellipsis font-semibold
          `}>
            {list.name}
          </div>
        </div>
        <div className='relative flex flex-row'>
          <div
            className={`
         ${isActive && 'text-white'} text-xs
          `}>
            {list.tokens.length} tokens
          </div>
          <div className='flex items-center justify-center' ref={node as any}>
            <button onClick={toggle} ref={setReferenceElement} style={{ padding: '0' }}>
            ...
            {open && (
              <div
                ...
                <button className='hover:text-high-emphesis text-primary' onClick={handleRemoveList} disabled={Object.keys(listsByUrl).length === 1}>
                  Remove list
                </button>
                {pending && (
