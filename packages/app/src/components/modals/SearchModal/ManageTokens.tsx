...
function ManageTokens({ setModalView, setImportToken }: { setModalView: (view: CurrencyModalView) => void; setImportToken: (token: Token) => void }) {
    ...
    return (
      ...
      userAddedTokens.map((token) => (
        <div key={token.address}>
          ...
          <div>
            <div className='flex items-center justify-center w-4 h-4 cursor-pointer hover:opacity-70' onClick={() => removeToken(chainId, token.address)}>
              <Trash />
            </div>
            <a href={getExplorerLink(chainId, token.address, 'address')} />
          </div>
