        ...
        <CardBody
          ...
          <Link to={`/feed/myzoo/${animal.tokenID}`}>
            <TextWrapper
              ...
              {animal.timeRemaining === 0 ? (group[animal.kind] > 1 ? `x${group[animal.kind]}` : '') : ''}
            </TextWrapper>
            <TextWrapper
              style={{
                textShadow: '0px 2px 6px rgb(0, 0, 0)',
          ...
          {animal.timeRemaining > 0 ? (
            ...
            <InfoBlock onClick={onInfoClick}>
              <BreedWrapper>{eggType === 'pure' ? `BREED` : animal.listed ? 'LISTED' : `SELL`}</BreedWrapper>
            </InfoBlock>
          )}
        </CardBody>
      </Card>
