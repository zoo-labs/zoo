...
const App: React.FC = () => {
  ...
  function requireLogin(component) { return (signedIn ? component : <Redirect to='/login'/>) }
      ...
      <Router history={history}>
        ...
        <Switch>
          ...
          <SuspenseWithChunkError fallback={<></>}>
            <Route path='/account'>{requireLogin(<Account/>)}</Route>
            <Route path='/feed'>{requireLogin(<Feed/>)}</Route>
            <Route path='/bank'>{requireLogin(<Bank/>)}</Route>
            <Route exact path='/'>{requireLogin(<Redirect to='/account'/>)}</Route>
