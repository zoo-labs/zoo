...
import DoubleCurrencyLogo from 'components/CurrencyInputPanel/DoubleLogo'
    ...
    >
      <button
        ...
        <div className='flex items-center space-x-4'>
          ...
          <div className='text-xl font-semibold'>{!currency0 || !currency1 ? <div>Loading</div> : `${currency0.symbol}/${currency1.symbol}`}</div>
        </div>
        <div className='flex items-center space-x-4'>
          Manage
          {showMore ? <ChevronUpIcon width='20px' height='20px' className='ml-4' /> : <ChevronDownIcon width='20px' height='20px' className='ml-4' />}
        </div>
      </button>
      ...
      <Transition
        ...
        <div className='p-4 space-y-4'>
          <div className='px-4 py-4 space-y-1 text-sm rounded text-high-emphesis bg-dark-900'>
            ...
            <div className='flex items-center justify-between'>
              <div>Your pool share:</div>
              <div className='font-semibold'>{poolTokenPercentage ? (poolTokenPercentage.toFixed(2) === '0.00' ? '<0.01' : poolTokenPercentage.toFixed(2)) + '%' : '-'}</div>
            </div>
          </div>
          {userDefaultPoolBalance && JSBI.greaterThan(userDefaultPoolBalance.quotient, BIG_INT_ZERO) && (
            <div className='grid grid-cols-2 gap-4'>
              <button
                color='blue'
                onClick={() => {
                  history.push(`/add/${pair.liquidityToken.address}`)
                }}>
                Add
              </button>
              <button
