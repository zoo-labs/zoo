...
class ZooFeed {
  constructor(elem, dataset){
    ...
    this.filterPosts(this.state.id, this.state.name)
  }
  ...
  filterPosts(postId, postName){
    ...
    if(postId){
      ...
      const idx = this.active_dataset.findIndex(t => t.name == postName)
      if(idx > -1){
        this.active_dataset.unshift(this.active_dataset.splice(idx, 1)[0])
      }
    }
  ...
  async addPage(creature){
    if(!creature) return;
    //else
    const page = document.createElement('div')

    if(creature.is_hybrid){
      creature.imgstr = window.cloudinaryImgURL(creature.hybrid_name_1, creature.name)
    } else {
...
;(async function(){
  const datasrc = window.userFeed ? `/data/creatures/${(JSON.parse(localStorage.czUser)).id}/true` : '/data/all-creatures',
    dataresp = await fetch(datasrc).then(resp => resp.json()),
    animalsShown = [],
    creatureFeed = dataresp.creatures.reduce((arr, creature) => {
    //for now only show one of each
    if(animalsShown.indexOf(creature.name) === -1){
      creature.birthdate = (new Date(creature.created)).toLocaleDateString()
      if(!creature.yield){
        creature.yield = {
          YTD: Math.floor(Math.random() * 3000) + 2000,
          daily: (Math.floor(Math.random() * 1000) + 2000) / 100
        }
      }

      animalsShown.push(creature.name)
      arr.push(creature)
    }

    return arr
  }, [])
  window.zoo = new ZooFeed(document.getElementById('ShoppableVid'), creatureFeed);
}());