...
contract ZooMedia is Media, Ownable {
    ...
    enum Animal{ PUG, BUTTERFLY, KITTEN, TURTLE, PENGUIN, DUCKLING, ORCA, ELK, PANDA, GORILLA, ELEPHANT, LION, BEAR, SHARK, BLOBFISH, NAKED_MOLE_RAT,
    ...
    constructor(string memory symbol, string memory name, address marketAddress) Media(symbol, name, marketAddress) {
        ...
        for (uint i=0; i < uint(NAKED_RAT_BABY) + 1; i++ ) {
            for (uint j=i; j < uint(NAKED_RAT_BABY) + 1; j++ ) {
    ...
    function concatAnimalIds(uint a1, uint a2) internal returns (string memory) {
        return string(abi.encodePacked(sha3(a1), sha3(a2)));
    }
