...
contract ZooMedia is IMedia, ERC721Burnable, ReentrancyGuard {
    ...
    modifier onlyApprovedOrOwner(address spender, uint256 tokenId) {
        ...
        require(_isApprovedOrOwner(spender, tokenId), "ZooMedia: Only approved or owner");
        _;
    }
     ...
     */
    function mintToken(address owner, Token memory token) public nonReentrant returns (Token memory) {
        _hashToken(token);
    ...
    */
    function _mintForCreator(
        ...
        require(
            bidShares.creator.value > 0,
            "ZooMedia: Ensure bidshares exists"
        );
        require(
            data.contentHash != 0,
