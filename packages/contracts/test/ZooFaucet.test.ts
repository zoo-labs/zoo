...
describe("Test Faucet", () => {
    ...
    it("Should be able withdraw ZOO from Faucet", async () => {
        ...
        expect(parseInt(ownerBalPostWithdrawTok)).to.equal(
            parseInt(ownerBal) + parseInt(faucetBalPostBuy));
        ...
        expect(parseInt(faucetBalPostBuy)).to.equal(
            parseInt(faucetInitialBal) - faucetWithdrawAmt);
