...
describe('ZooMarket', () => {
  ...
  describe('#setBid', () => {
    ...
    it('should refund the original bid if the bidder bids again', async () => {
      ...
      const bidderBalance = toNumWei(await ZooToken__factory.connect(defaultBid.currency, bidderWallet).balanceOf(bidderWallet.address))
      ...
      const afterBalance = toNumWei(await ZooToken__factory.connect(defaultBid.currency, bidderWallet).balanceOf(bidderWallet.address))
