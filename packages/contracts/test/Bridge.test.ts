...
describe.only('Bridge', function () {
  ...
  describe('swap', async () => {
    ...
    it('cannot swap a disabled token', async () => {
      ...
      await expect(bridge.swap(tokenA, tokenB, user1.address, 100, 1)).to.be.rejectedWith('Swap from token not enabled')
    })
    ...
    it('cannot swap to a disabled token', async () => {
      ...
      await expect(bridge.swap(tokenA, tokenB, user1.address, 100, 1)).to.be.rejectedWith('Swap to token not enabled')
    })
    ...
    it('can only swap a greater than zero amount', async () => {
      ...
      await expect(bridge.swap(tokenA, tokenB, user1.address, 0, 2)).to.be.rejectedWith('Amount must be greater than zero')
    })
    ...
    it('can only to a non-zero account', async () => {
      ...
      await expect(bridge.swap(tokenA, tokenB, user1.address, 0, 1)).to.be.rejectedWith('Amount must be greater than zero')
    })
