...
describe.only('ZooMedia', () => {
  ...
  async function setAsk(media: ZooMedia, tokenId: number, ask: Ask) {
    return media.setAsk(tokenId, ask);
  }

  async function removeAsk(media: ZooMedia, tokenId: number) {
    return media.removeAsk(tokenId);
  }

  async function setBid(media: ZooMedia, bid: Bid, tokenId: number) {
    return media.setBid(tokenId, bid);
  }

  async function removeBid(media: ZooMedia, tokenId: number) {
    return media.removeBid(tokenId);
  }

  async function acceptBid(media: ZooMedia, tokenId: number, bid: Bid) {
    return media.acceptBid(tokenId, bid);
  }

  // Trade a token a few times and create some open bids
  async function setupAuction(tokenAddress: string, tokenId = 0) {
    ...
    await mintCurrency(tokenAddress, creatorWallet.address, 10000);
    await mintCurrency(tokenAddress, prevOwnerWallet.address, 10000);
    await mintCurrency(tokenAddress, ownerWallet.address, 10000);
    await mintCurrency(tokenAddress, bidderWallet.address, 10000);
    await mintCurrency(tokenAddress, otherWallet.address, 10000);
    await approveCurrency(tokenAddress, marketAddress, creatorWallet);
    await approveCurrency(tokenAddress, marketAddress, prevOwnerWallet);
    await approveCurrency(tokenAddress, marketAddress, ownerWallet);
    await approveCurrency(tokenAddress, marketAddress, bidderWallet);
    await approveCurrency(tokenAddress, marketAddress, otherWallet);
    ...
    await setBid(
      asPrevOwner,
      defaultBid(tokenAddress, prevOwnerWallet.address),
      tokenId
    );
    ...
    await setBid(
      asOwner,
      defaultBid(tokenAddress, ownerWallet.address),
      tokenId
    );
    await acceptBid(
      asPrevOwner,
    ...
    await setBid(
      asBidder,
      defaultBid(tokenAddress, bidderWallet.address),
      tokenId
    );
    await setBid(
      asOther,
      defaultBid(tokenAddress, otherWallet.address),
      tokenId
    );
  }
  ...
  describe('#mint', () => {
    ...
    // TODO: fix this
    it.skip('should revert if the content hash already exists for a created token', async () => {
      const media = await mediaAs(creatorWallet);
  ...
  describe('#mintWithSig', () => {
    ...
    it.skip('should not mint a token for a different creator', async () => {
      const media = await mediaAs(otherWallet);
      const sig = await signMintWithSig(
        bidderWallet,
        media.address,
    ...
    it.skip('should not mint a token for a different contentHash', async () => {
    ...
    it.skip('should not mint a token for a different creator bid share', async () => {
      const media = await mediaAs(otherWallet);
      const sig = await signMintWithSig(
        creatorWallet,
        media.address,
    ...
    it.skip('should not mint a token with an invalid deadline', async () => {
      const media = await mediaAs(otherWallet);
      const sig = await signMintWithSig(
        creatorWallet,
        media.address,
  ...
  describe('#setAsk', () => {
    ...
    beforeEach(async () => {
      await deploy();
      tokenAddress = await deployCurrency();
      await setupAuction(tokenAddress);
    });
  ...
  describe('#setBid', () => {
    ...
    it.skip('should revert if the token bidder does not have a high enough allowance for their bidding currency', async () => {
      const media = await mediaAs(bidderWallet);
      await expect(
        media.setBid(0, defaultBid(tokenAddress, bidderWallet.address))
      ).rejectedWith('SafeERC20: ERC20 operation did not succeed');
    });

    it.skip('should revert if the token bidder does not have a high enough balance for their bidding currency', async () => {
      const media = await mediaAs(bidderWallet);
      await approveCurrency(tokenAddress, marketAddress, bidderWallet);
      await expect(
        media.setBid(0, defaultBid(tokenAddress, bidderWallet.address))
    ...
    it('should set a bid', async () => {
      const media = await mediaAs(bidderWallet);
      await approveCurrency(tokenAddress, marketAddress, bidderWallet);
      await mintCurrency(tokenAddress, bidderWallet.address, 100000);
      await expect(
        media.setBid(0, defaultBid(tokenAddress, bidderWallet.address))
      ).fulfilled;
      const balance = await getBalance(tokenAddress, bidderWallet.address);
    ...
    it('should automatically transfer the token if the ask is set', async () => {
      const media = await mediaAs(bidderWallet);
      const asOwner = await mediaAs(ownerWallet);
      await setupAuction(tokenAddress, 1);
      await setAsk(asOwner, 1, { ...defaultAsk, currency: tokenAddress });

      await expect(
        media.setBid(1, defaultBid(tokenAddress, bidderWallet.address))
      ).fulfilled;
    ...
    it('should refund a bid if one already exists for the bidder', async () => {
      ...
      const beforeBalance = toNumWei(
        await getBalance(tokenAddress, bidderWallet.address)
      );
      await setBid(
        media,
        {
  ...
  describe('#acceptBid', () => {
    ...
    it('should accept a bid', async () => {
      ...
      const asBidder = await mediaAs(bidderWallet);
      const bid = {
        ...defaultBid(tokenAddress, bidderWallet.address, otherWallet.address),
        sellOnShare: Decimal.new(15),
      };
      await setBid(asBidder, bid, 0);

      const beforeOwnerBalance = toNumWei(
        await getBalance(tokenAddress, ownerWallet.address)
      );
      const beforePrevOwnerBalance = toNumWei(
        await getBalance(tokenAddress, prevOwnerWallet.address)
      );
      const beforeCreatorBalance = toNumWei(
        await getBalance(tokenAddress, creatorWallet.address)
      );
    ...
    it('should emit a bid finalized event if the bid is accepted', async () => {
      ...
      const media = await mediaAs(ownerWallet);
      const auction = await ZooMarket__factory.connect(marketAddress, bidderWallet);
      const bid = defaultBid(tokenAddress, bidderWallet.address);
      const block = await provider.getBlockNumber();
      await setBid(asBidder, bid, 0);
    ...
    it('should revert if not called by the owner', async () => {
      ...
      await expect(
        media.acceptBid(0, { ...defaultBid(tokenAddress, otherWallet.address) })
    ...
    it('should revert if a non-existent bid is accepted', async () => {
      const media = await mediaAs(ownerWallet);
      await expect(
        media.acceptBid(0, { ...defaultBid(tokenAddress, AddressZero) })
  ...
  describe('#burn', () => {
    ...
    it('should revert when the caller is the owner, but not creator', async () => {
      const creatorToken = await mediaAs(creatorWallet);
    ...
    it('should revert if the token id does not exist', async () => {
      ...
      await expect(media.burn(100)).rejectedWith('Media: nonexistent token');
    });
    ...
    it('should clear approvals, set remove owner, but maintain tokenURI and contentHash when the owner is creator and caller', async () => {
      ...
      const mediaURI = await media.tokenURI(0);
  ...
  describe('#updateMetadataURI', async () => {
    ...
    it.skip('should set the tokenMetadataURI to the URI passed if msg.sender is the owner', async () => {
      ...
      const mediaURI = await media.tokenMetadataURI(0);
      expect(tokenURI).eq('blah blah');
    });
