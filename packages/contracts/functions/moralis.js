...
Moralis.Cloud.afterSave("Hatch", async (request) => {
	const confirmed = request.object.get("confirmed");
      ...
      current.set("metadataURI", hatchedAnimal.data.metadataURI)
      current.set("Rarity", hatchedAnimal.rarity.name)
      current.set("Yield", parseInt(hatchedAnimal.rarity.yield))
      current.set("Boost", parseInt(hatchedAnimal.rarity.boost))
      current.set("Name", hatchedAnimal.name)
      current.set("Timestamp", parseInt(hatchedAnimal.timestamp))
      current.set("Listed", false)
      current.set("Revealed", false)
      current.set("Kind", parseInt(hatchedAnimal.kind))
