...
const AssetSale = (props: AssetSaleProps) => {
  ...
  return (
    <div
      className={`AssetSale ${props.className || ""} ${
        props.onClickTokenType ? "cursor-pointer" : ""
      }`}
      onClick={() =>
        props.onClickTokenType && props.onClickTokenType(props.name)
      }
    >
      {props.large && video ? (
        <Player
        ...
        image && (
          <Image layout="fill" src={image} alt={`${type} ${dropId}-${name}`} />
        )
      )}
      <div
      ...
      >
        <div>
          ...
          <br />
          <span className="px-2 py-1 ml-2 text-xs font-bold text-black bg-gray-300 rounded-full zoo-font AssetSale__token-id">
            {minted} / {supply} Sold
          </span>
        </div>
              ...
              <div className="grid grid-cols-2 px-3 py-2 my-3 bg-indigo-300 rounded-lg">
                ...
                <div className="flex justify-end">
                  <div className="text-right ">
                    ...
                    {usdAmount && (
                      <small className="text-gray-700">${usdAmount}</small>
                    )}
                  </div>
                  {
