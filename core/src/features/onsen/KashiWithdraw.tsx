...
const KashiWithdraw = ({ pair, useBento }) => {
  ...
  const amountAvailable = minimum(
    pair?.maxAssetAvailable ?? ZERO,
    pair?.currentUserAssetAmount.value ?? ZERO
  );
  const available =
    assetToken &&
    amountAvailable &&
    CurrencyAmount.fromRawAmount(assetToken, amountAvailable.toString());
  ...
  async function onWithdraw(cooker: KashiCooker): Promise<string> {
    const maxFraction = minimum(
      pair.userAssetFraction,
      pair.maxAssetAvailableFraction
    );
    const fraction = BigNumber.from(
